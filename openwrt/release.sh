...
for f in etc/opkg/keys/$($HOSTBIN/usign -F -p $K/key-build.pub) etc/apk/keys/public-key.pem usr/bin/ucert usr/bin/usign usr/bin/fwtool usr/bin/scp usr/sbin/fw_setenv usr/sbin/neon-role usr/sbin/neon-watchdog usr/sbin/neon-led usr/sbin/neon-ota usr/sbin/neon-node usr/sbin/neon-fleet www/cgi-bin/neon-node usr/bin/iperf3 etc/init.d/neon-watchdog etc/init.d/neon-ota etc/rc.d/S99neon-ota etc/config/neon_ota www/luci-static/resources/view/neon/ota.js usr/share/luci/menu.d/luci-app-neon-ota.json usr/share/rpcd/acl.d/luci-app-neon-ota.json www/luci-static/resources/view/status/include/05_neon_mesh.js usr/share/rpcd/acl.d/luci-app-neon-mesh.json etc/uci-defaults/50-neon-mesh lib/upgrade/keep.d/neon-mesh etc/neon-release etc/apk/repositories.d/neon.list etc/uci-defaults/30_uboot-envtools; do
